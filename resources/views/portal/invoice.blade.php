...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .topbar { background: linear-gradient(135deg, #002045 0%, #1a365d 100%); padding: 20px 32px; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
        .topbar-brand { color: #fff; font-size: 18px; font-weight: 800; }
        .topbar-sub { color: rgba(255,255,255,0.6); font-size: 12px; margin-top: 2px; }
        .topbar-actions { display: flex; gap: 8px; flex-shrink: 0; }
        a.btn { display: inline-block; padding: 8px 18px; background: #fff; color: #002045; border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 800; }
        a.btn-outline { background: transparent; color: #fff; border: 2px solid rgba(255,255,255,0.5); }
        a.btn:hover { opacity: 0.88; }
        .container { max-width: 860px; margin: 0 auto; padding: 32px 16px; }
        .card { background: #fff; border-radius: 16px; padding: 32px; margin-bottom: 24px; box-shadow: 0 2px 12px rgba(0,32,69,0.07); }
        .card-title { font-size: 13px; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; color: #57657a; margin-bottom: 16px; }
        .meta-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        @media (max-width:600px) { .meta-grid { grid-template-columns: 1fr; } }
        .meta-label { font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.12em; color: #57657a; margin-bottom: 4px; }
        .meta-value { font-size: 15px; font-weight: 700; color: #002045; }
        .badge { display: inline-block; padding: 3px 12px; border-radius: 999px; font-size: 11px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; }
        ...
        table.items th { padding: 10px 12px; text-align: left; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; color: #57657a; background: #f8faff; }
        ...
        .payments-list li { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f0f4f9; font-size: 14px; }
        .payments-list li:last-child { border-bottom: none; }
        footer { text-align: center; padding: 32px 16px; font-size: 12px; color: #aab0bc; }
        .bank-box { background: #f8faff; border-radius: 10px; padding: 16px; margin-top: 16px; }
        .bank-box p { font-size: 13px; color: #374151; line-height: 1.8; }
    </style>
</head>
<body>
    <div class="topbar">
        ...
        <div class="topbar-actions">
            ...
            <a href="{{ route('portal.invoice.pdf', ['token' => $token, 'invoice' => $invoice]) }}" class="btn">⬇ Télécharger PDF</a>
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="bank-box">
                <p>
                    ...
                    @if($bankDetails['account_name'])<strong>Intitulé :</strong> {{ $bankDetails['account_name'] }}<br>@endif
                    @if($bankDetails['account_number'])<strong>Numéro de compte :</strong> {{ $bankDetails['account_number'] }}<br>@endif
